      ...
      switch(msg.type) {
        ...
        case "username":
	      ...
	      case "video-answer":
		   ...
		   //if that person already have video chat, the one who contacts it is not 
		   //a broadcast initiator and have to take the broadcastId of already exested 
		   //videochat
		 ...
		 if(arr.name == msg.target){
		  ...
		  var narr = arr;
		   ...
		   for(i=0;i<narr.relayReceivers.length; i++){
			   if(narr.relayReceivers[i] != msg.name){
		   ...
		   broadId = msg.name;
		   relayReceivers.push(msg.name);
		   relayReceivers.push(msg.target);
	     }
